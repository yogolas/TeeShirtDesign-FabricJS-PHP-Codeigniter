...
<div class="panel panel-default">
    ...
    <li role="presentation" class="waitprocess active"><a href="#waitprocess" aria-controls="waitprocess" role="tab" data-toggle="tab">Campaign Wait For packaging</a></li>
    <li role="presentation"><a href="#process" aria-controls="process" role="tab" data-toggle="tab">Campaign packaging</a></li>
  </ul>
...
<table class="table table-hover table-bordered">
	...
	<tbody>
		<tr ng-repeat="x in campaignlist | filter:searchcid">
			...
			<td>
<span ng-if="x.campaign_status=='3'" class="label label-warning">รอแพ็ค</span>
			</td>
			<td>
				<button class="btn btn-success" ng-click="Okthisprocess(x)">เริ่มแพ็ค</button>
...
<table class="table table-hover table-bordered">
	...
	<tbody>
		<tr ng-repeat="x in campaignprocesslist | filter:searchcid2">
			...
			<td>
<span ng-if="x.campaign_status=='4'" class="label label-warning">กำลังแพ็ค</span>
			</td>
			<td>
				<button class="btn btn-success" ng-click="Okthisprocesssuccess(x)">แพ็คเรียบร้อย</button>
...
<td>{{x.qty}}</td>
<td>
				<button class="btn btn-default" ng-click="Printaddressandorder(x)">
					Open Print
				</button>
			</td>
			<td>
			...
			<td><button ng-show="x.odstatus=='0'" class="btn btn-success" ng-click="Okthisdetailpackok(x)">แพ็คแล้ว</button></td>
...
<div class="modal fade" id="Openprint">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				<h4 class="modal-title">Order No: {{order_number}}</h4>
				<button class="btn btn-warning" ng-click="Print()">Print</button>
			</div>
...
<table class="table table-hover" style="width: 100%;">
	...
	<tbody>
		<tr ng-repeat="x in orderlistprint" style="font-size: 20px;">
			<td>{{x.product_style_name}}</td>
			<td>{{x.product_size_name}}</td>
			<td>{{x.product_color_name}}</td>
			<td>{{x.qty}}</td>
...
$scope.Printaddressandorder = function(x){
$('#Openprint').modal('show');
$scope.order_number = x.order_number;
$scope.name = x.name;
$scope.address = x.address;
$scope.province = x.province;
...
order_number: x.order_number
       ...
       $scope.orderlistprint = response.data;
